    ...
    if (row) {
      ...
      bcrypt.hash(password, 10, (err, hash) => {
        if (err) {
          ...
          db.run('INSERT INTO users (cpf, username, password, email, phone, sexo, curso) VALUES (?,?,?,?,?,?,?)', [cpf, username, hash, email, phone, sexo, curso], (err) => {
            if (err) {
...
router.put('/:id', verifyJWT, function (req, res, next) {
  const { id } = req.params;
  const { cpf, username, password, email, phone, sexo, curso } = req.body;
  db.run('UPDATE users SET cpf = ?, username = ?, password = ?, email = ?, phone = ?, sexo = ?, curso = ? WHERE id = ?', [cpf, username, password, email, phone, sexo, curso, id], function (err) {
    if (err) {
